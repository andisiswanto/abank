	...
	<head>
        ...
        <script>
            function cekDetail(kode){
                $.post("<?= site_url() ?>/administrator/c_contact/kontakDetail", {
                    kode : kode
                },
                function(data) {
                    $('#isiModal').html(data);
            ...
            function edit(kode){
                $.post("<?= site_url() ?>/administrator/c_contact/kontakEdit", {
                    kode : kode
                },
                function(data) {
                    $('#isiModal').html(data);
            ...
            function add(){
                $.post("<?= site_url() ?>/administrator/c_contact/kontakAdd", {
                },
                function(data) {
                    $('#page').html(data);
                    //$('#detailModal').modal('show');
        ...
        <table id="example" class="display" cellspacing="0" width="100%">
            ...
            <tbody>
                ...
                foreach($tabel as $tbl){
                    $ctcid=$tbl->ctcid;
                    $ctcname=$tbl->ctcname;
                    $ctcaddress=$tbl->ctcaddress;
                    $ctcphone=$tbl->ctcphone;
                    $ctcemail=$tbl->ctcemail;
                    $ctcfax=$tbl->ctcfax;
                    $ctczip=$tbl->ctczip;
                    $ctcdeskripsi=$tbl->ctcdeskripsi;
                    ?>
                    <tr>
                        <td><?php echo $ctcid;?></td>
                        <td><?php echo $ctcname;?></td>
                        <td>
                            <button class="btn btn-info" id="detail" onclick="cekDetail('<?php echo $ctcid;?>')">DETAIL</button>
                            <button class="btn btn-warning" id="edit" onclick="edit('<?php echo $ctcid;?>')">EDIT</button>
                        </td>
                    </tr>  
                    <?php
                }
